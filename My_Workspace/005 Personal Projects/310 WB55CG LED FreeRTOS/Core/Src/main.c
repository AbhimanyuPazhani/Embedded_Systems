...
int main(void)
{
  ...
  /* add queues, ... */
  myQueue = osMessageQueueNew(5, sizeof(uint8_t), NULL);
  ...
  GreenTaskHandle = osThreadNew(StartTask02, NULL, &GreenTask_attributes);

  /* creation of BlueTask */
  BlueTaskHandle = osThreadNew(StartTask03, NULL, &BlueTask_attributes);

  /* creation of RedTask */
  RedTaskHandle = osThreadNew(StartTask04, NULL, &RedTask_attributes);
...
void StartTask04(void *argument)
{
  ...
  for(;;)
  {
    if(osMessageQueueGet(myQueue, &rxData, NULL, osWaitForever) == osOK)
    {
      HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_1);
      osDelay(10);  //  ADD THIS LINE
    }
  }
}
